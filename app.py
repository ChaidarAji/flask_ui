...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2[['Category', 'App', 'Reviews', 'Rating']].sort_values('Reviews', ascending=False)[:10].reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    x = (playstore.sort_values('Installs', ascending=False).head(100).Size/1000000).values
    y = (playstore.sort_values('Installs', ascending=False).head(100).Installs/10000000).values
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    plt.scatter(x=x,y=y, alpha=0.3)
    plt.xlabel('Size')
    plt.ylabel('Installs (M)')
    plt.savefig('size_install.png',bbox_inches="tight")
